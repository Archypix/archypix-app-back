...
pub struct User {
    ...
    pub tfa_login: bool,
    pub storage_count_ko: i64,
    pub storage_limit_ko: i64,
}

impl User {
    pub fn from_id(conn: &mut DBConn, id: &i32) -> Result<User, ErrorResponder> {
        User::from_id_opt(conn, id).and_then(|user_opt| user_opt.ok_or_else(|| ErrorType::UserNotFound.res()))
    }
    pub fn from_id_opt(conn: &mut DBConn, id: &i32) -> Result<Option<User>, ErrorResponder> {
        users::table
    ...
    pub fn find_logged_in(conn: &mut DBConn, user_id: i32, auth_token: Vec<u8>) -> Result<(User, AuthToken), ErrorResponder> {
        User::find_logged_in_opt(conn, user_id, auth_token).and_then(|data| data.ok_or_else(|| ErrorType::UserNotFound.res()))
    }
    pub fn find_logged_in_opt(conn: &mut DBConn, user_id: i32, auth_token: Vec<u8>) -> Result<Option<(User, AuthToken)>, ErrorResponder> {
        users::table
    ...
    pub(crate) fn create_user(conn: &mut DBConn, name: &str, email: &str, password: &str) -> Result<i32, ErrorResponder> {
    ...
    pub fn switch_status_from_id(conn: &mut DBConn, user_id: &i32, status: &UserStatus) -> Result<(), ErrorResponder> {
        update(users::table)
            .filter(users::dsl::id.eq(user_id))
    ...
    pub fn get_id_from_headers(request: &Request<'_>) -> Option<i32> {
        request.headers().get_one("X-User-Id").map(|s| s.parse::<i32>().ok()).flatten()
    }
}
